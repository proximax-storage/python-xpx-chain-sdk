...
class TestBlockchainHttp(harness.TestCase):
    ...
    async def test_get_block_by_height(self, data, cb):
        http = data[0](responses.ENDPOINT)

        with data[1].default_response(200, **responses.BLOCK_INFO["Ok"]):
            block_info = await cb(http.get_block_by_height(1))
            ...
            self.assertEqual(block_info, await cb(http.getBlockByHeight(1)))
