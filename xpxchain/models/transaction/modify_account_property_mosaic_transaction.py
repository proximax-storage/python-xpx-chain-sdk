"""
    modify_account_property_mosaic_transaction
    ==========================================

    Transaction to modify account property mosaics.
...
def to_catbuffer(modification: AccountPropertyModification) -> bytes:
    ...
    value = util.u64_to_catbuffer(int(typing.cast(MosaicId, modification.value)))
    return type + value
...
def from_catbuffer(data: bytes) -> AccountPropertyModification:
    ...
    value = MosaicId(util.u64_from_catbuffer(data[1:9]))
    return AccountPropertyModification(modification_type, value)


@util.inherit_doc
@util.dataclass(frozen=True)
@register_transaction('MODIFY_ACCOUNT_PROPERTY_MOSAIC')
class ModifyAccountPropertyMosaicTransaction(Transaction):
    """
    Modify account property mosaics transaction.
    ...
    def __init__(
        ...
        if not all(i.is_mosaic() for i in modifications):
            raise ValueError('Modify account property mosaics got invalid value.')
        super().__init__(
    ...
    def catbuffer_size_specific(self) -> int:
        ...
        modification_size = util.U8_BYTES + util.U64_BYTES
    ...
    def to_catbuffer_specific(
        ...
        """Export modify account mosaics-specific data to catbuffer."""

        # uint8_t property_type
        # uint8_t modifications_count
        # typedef Modification { type: uint8_t, value: uint64_t }
    ...
    def load_catbuffer_specific(
        ...
        """Load modify account mosaics-specific data data from catbuffer."""
...
@register_transaction('MODIFY_ACCOUNT_PROPERTY_MOSAIC')
class ModifyAccountPropertyMosaicInnerTransaction(
    InnerTransaction,
    ModifyAccountPropertyMosaicTransaction
):
    """Embedded modify account mosaics transaction."""
